...
try {
    ...
    try {
        ...
        if (addPayment($data) !== false && $data['payment_status'] === 'approved') {
            ...
            header("location:http://localhost/projects/project/payment/PaypalSuccess.php?payid=$inserids");
            exit(1);
        } else {
            // Payment failed
            header("location:http://localhost/projects/project/payment/PaypalFailed.php");
            exit(1);
        }
    } catch (Exception $e) {
